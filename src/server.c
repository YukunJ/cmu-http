...
int main(int argc, char *argv[]) {
  ...
  int max_pending_queue = 1024; /* how many pending connection allowed to be
  ...
  add_to_poll_array(listen_fd, poll_array,
                    POLLIN);  // add the listening fd to be polled
            ...
            if (poll_array->sizes[i] > 0) {
              if (result_code == TEST_ERROR_PARSE_PARTIAL) {
                ...
                for (int request_counter = 0;
                     request_counter < request.header_count;
                     request_counter++) {
                  ...
                  if (strcasecmp(request.headers[request_counter].header_name,
                                 "Connection") == 0) {
                    if (strcasecmp(
                            request.headers[request_counter].header_value,
                            CONNECTION_CLOSE) == 0) {
                      should_close = true;
                    }
                  }
                ...
                bool is_bad_request = serve_request(
                    ready_fd, &request, www_folder, poll_array->buffers[i],
                    read_amount, should_close);
                // free(request.headers);
                if (request.body != NULL) {
                  free(request.body);
                }
                // if the request has 'Connection: close' in header or the
                // request is bad should close the connection after service
                // immediately
