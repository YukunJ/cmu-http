...
/**
 * @brief load the file into a populated buffer and overwrite the size to tell
 * outside world
 ...
 * @param size the pointer to size_t indicating how many bytes are loaded from
 * the file
 */
 ...
 */
bool serve_request(int client_fd, Request *request, const char *server_dir,
                   const char *read_buf, int read_amount, bool should_close) {
  assert(request->valid == true);
  /* check if the HTTP version if the request is 1.1 */
  if (strcmp(request->http_version, HTTP_VER) != 0) {
    char *response;
    size_t response_len;
    serialize_http_response(&response, &response_len, BAD_REQUEST_SHORT, NULL,
                            NULL, NULL, 0, NULL, should_close);
  ...
  if (strcmp(request->http_method, GET) == 0 ||
    ...
    if (check_file_existence(whole_path)) {
      ...
      strftime(last_modified, sizeof(last_modified), "%a, %d %b %Y %H:%M:%S %Z",
               &gmt_time);

      char *response;
      size_t response_len;
      if (strcmp(request->http_method, GET) == 0) {
        serialize_http_response(&response, &response_len, OK, extension,
...
int main(int argc, char *argv[]) {
  ...
  int max_pending_queue = 200; /* how many pending connection allowed to be
                                  placed in the socket queue */
  ...
  // add_to_poll_array(listen_fd, poll_array, POLLIN); // add the listening fd
  // to be polled
  ...
  while (true) {
    ...
    while (poll(listen_pfds, 1, 0)) {
      ...
      int new_client_fd =
          accept(listen_fd, (struct sockaddr *)&their_addr, &sin_size);
      if (poll_array->count >= MAX_CONNECTION) {
        /* send 503 and close it */
        char *response;
        size_t response_len;
        serialize_http_response(&response, &response_len, SERVICE_UNAVAILABLE,
        ...
        if (poll_array->pfds[i].revents & POLLIN) {
          ...
          if (ready < 0) {
            ...
            if (poll_array->sizes[i] == 0) {
              ...
              poll_array->buffers[i] = (char *)realloc(
                  poll_array->buffers[i],
                  (ready + poll_array->sizes[i]) * sizeof(char));
              memcpy(poll_array->buffers[i] + poll_array->sizes[i], local_buf,
                     ready);
              poll_array->sizes[i] += ready;
            }
          }
          // try to parse data to see if we have valid requests, and respond
          // accordingly use while loop to handle multiple requests
          ...
          while (poll_array->sizes[i] > 0) {
            if (result_code == TEST_ERROR_PARSE_PARTIAL) {
              ...
              for (int request_counter = 0;
                   request_counter < request->header_count; request_counter++) {
                if (strcasecmp(request->headers[request_counter].header_name,
                               "Content-Length") == 0) {
                  size_t content_len;
                  sscanf(request->headers[request_counter].header_value, "%zu",
                         &content_len);
                  ...
                  memcpy(request->body, poll_array->buffers[i] + read_amount,
                         content_len);
                  // printf("request body: %s\n", request.body);
                  read_amount += content_len;
                }
                if (strcasestr(request->headers[request_counter].header_name,
                               "Connection") != NULL) {
                  if (strcasestr(request->headers[request_counter].header_value,
              ...
              serve_request(ready_fd, request, www_folder,
                            poll_array->buffers[i], read_amount, should_close);
              ...
              free(request);
              // if the request has 'Connection: close' in header or the request
              // is bad should close the connection after service immediately
              if (should_close) {
              ...
              char *response;
              size_t response_len;
              serialize_http_response(&response, &response_len,
                                      BAD_REQUEST_SHORT, NULL, NULL, NULL, 0,
                                      NULL, true);
            ...
            if (read_amount == poll_array->sizes[i]) {
              ...
              char *new_buffer = (char *)malloc(
                  (poll_array->sizes[i] - read_amount) * sizeof(char));
            ...
            if (poll_array->sizes[i] > 0) {
              ...
              memset(request, 0, sizeof(Request));
              result_code = parse_http_request(poll_array->buffers[i],
                                               poll_array->sizes[i], request,
                                               &read_amount);
            }
          }
        }
